...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-vector/build/test/createRandomStateTest\" vector/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pyvector
	OUTPUT_PYTHON="$(docker run --rm vector/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm vector/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm vector/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
