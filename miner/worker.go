...
func newWorker(coinbase common.Address, vec core.Backend) *worker {
	worker := &worker{
		vec:            vec,
		mux:            vec.EventMux(),
		chainDb:        vec.ChainDb(),
...
func (self *worker) setVecbase(addr common.Address) {
	self.mu.Lock()
	...
	for {
		for result := range self.recv {
			...
			if self.fullValidation {
				...
				auxValidator := self.vec.BlockChain().AuxValidator()
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	state, err := state.New(parent.Root(), self.vec.ChainDb())
	if err != nil {
		return err
	}
	work := &Work{
	...
	accounts, _ := self.vec.AccountManager().Accounts()
...
func (self *worker) commitNewWork() {
	...
	/* //approach 1
	transactions := self.vec.TxPool().GetTransactions()
	...
	*/
	...
	for _, tx := range self.vec.TxPool().GetTransactions() {
	...
	*/
	...
	self.vec.TxPool().RemoveTransactions(work.lowGasTxs)
