...
// Package miner implements Vector block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whvec we can start the mining operation
	shouldStart int32 // should start indicates whvec we should start after sync
}

func New(vec core.Backend, mux *event.TypeMux, pow pow.PoW) *Miner {
	miner := &Miner{vec: vec, mux: mux, pow: pow, worker: newWorker(common.Address{}, vec), canStart: 1}
	go miner.update()
...
func (self *Miner) SetVecbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setVecbase(addr)
}
