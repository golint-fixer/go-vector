...
func main() {
	...
	// Create an Vector peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting Vector peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start Vector peer: %v.\n", err)
		os.Exit(1)
	}

	// Send a message to self to check that somvecing works
