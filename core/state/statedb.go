...
// Package state provides a caching layer atop the Vector state trie.
package state
...
// StateDBs within the vector protocol are used to store anything
...
func New(root common.Hash, db vecdb.Database) (*StateDB, error) {
...
// NewStateObject create a state object whvec it exist in the trie or not
...
func (s *StateDB) CommitBatch() (root common.Hash, batch vecdb.Batch) {
	batch = s.db.NewBatch()
