...
// genTxRing returns a block generator that sends vec in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db vecdb.Database
	if !disk {
		db, _ = vecdb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "vec-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = vecdb.NewLDBDatabase(dir, 0)
		if err != nil {
