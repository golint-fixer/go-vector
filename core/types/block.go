...
// Package types contains data types related to Vector consensus.
package types
...
// a block's data contents (transactions and uncles) togvec.
type Body struct {
...
type Block struct {
	...
	// ReceivedAt is used by package vec to track block propagation time.
...
// [deprecated by vec/63]
// StorageBlock defines the RLP encoding of a Block stored in the
// state database. The StorageBlock encoding contains fields that
// would otherwise need to be recomputed.
type StorageBlock Block

// "external" block encoding. used for vec protocol, etc.
