...
func GetNotice(xvec *xvec.XEth, tx string, http *httpclient.HTTPClient) (notice string) {
	ns, err := New(xvec, tx, http)
	if err != nil {
		if ns == nil {
...
func New(xvec *xvec.XEth, jsontx string, http *httpclient.HTTPClient) (self *NatSpec, err error) {
	...
	content, err := FetchDocsForContract(contractAddress, xvec, http)
	if err != nil {
		return
	}
...
func FetchDocsForContract(contractAddress string, xvec *xvec.XEth, client *httpclient.HTTPClient) (content []byte, err error) {
	// retrieve contract hash from state
	codehex := xvec.CodeAt(contractAddress)
	codeb := xvec.CodeAtBytes(contractAddress)
...
func (self *NatSpec) makeAbi2method(abiKey [8]byte) (mvec *method) {
...
func (self *NatSpec) Notice() (notice string, err error) {
	...
	mvec := self.makeAbi2method(abiKey)

	if mvec == nil {
		err = fmt.Errorf("abi key does not match any method")
		return
	}
	notice, err = self.noticeForMethod(self.tx, mvec.name, mvec.Notice)
	return
}
