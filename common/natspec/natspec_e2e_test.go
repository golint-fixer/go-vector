...
func (self *testFrontend) ConfirmTransaction(tx string) bool {
	if self.wantNatSpec {
		...
		self.lastConfirm = GetNotice(self.xvec, tx, client)
	}
	return true
}
...
func testEth(t *testing.T) (vector *vec.Vector, err error) {
	...
	return vec.New(&vec.Config{
		DataDir:                 tmp,
		AccountManager:          am,
		Vecbase:               common.HexToAddress(testAddress),
		MaxPeers:                0,
		PowTest:                 true,
...
func testInit(t *testing.T) (self *testFrontend) {
	...
	self.wait = self.xvec.UpdateState()
	addr, _ := self.vector.Vecbase()
	...
	recG := self.xvec.GetTxReceipt(common.HexToHash(txG))
	if recG == nil {
	...
	recH := self.xvec.GetTxReceipt(common.HexToHash(txH))
	if recH == nil {
	...
	recU := self.xvec.GetTxReceipt(common.HexToHash(txU))
	if recU == nil {
