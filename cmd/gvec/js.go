...
import (
	...
	re "github.com/vector/go-vector/jsre"
	"github.com/vector/go-vector/rpc"
	"github.com/vector/go-vector/rpc/api"
...
func newLightweightJSRE(docRoot string, client comms.VectorClient, datadir string, interactive bool) *jsre {
	js := &jsre{ps1: "> "}
...
func newJSRE(vector *vec.Vector, docRoot, corsDomain string, client comms.VectorClient, interactive bool, f xvec.Frontend) *jsre {
	js := &jsre{vector: vector, ps1: "> "}
	// set default cors domain used by startRpc from CLI flag
	js.corsDomain = corsDomain
	if f == nil {
		f = js
	}
	js.xvec = xvec.New(vector, f)
	js.wait = js.xvec.UpdateState()
	js.client = client
	if clt, ok := js.client.(*comms.InProcClient); ok {
		if offeredApis, err := api.ParseApiString(shared.AllApis, codec.JSON, js.xvec, vector); err == nil {
	...
	if !liner.TerminalSupported() || !interactive {
		...
		lr := liner.NewLiner()
		js.withHistory(vector.DataDir, func(hist *os.File) { lr.ReadHistory(hist) })
		lr.SetCtrlCAborts(true)
		js.loadAutoCompletion()
		lr.SetWordCompleter(apiWordCompleter)
		lr.SetTabCompletionStyle(liner.TabPrints)
		js.prompter = lr
		js.atexit = func() {
			js.withHistory(vector.DataDir, func(hist *os.File) { hist.Truncate(0); lr.WriteHistory(hist) })
			lr.Close()
			close(js.wait)
		}
	}
...
// show summary of current gvec instance
func (self *jsre) welcome() {
	self.re.Run(`
    (function () {
      console.log('instance: ' + web3.version.client);
      console.log(' datadir: ' + admin.datadir);
      console.log("coinbase: " + vec.coinbase);
      var ts = 1000 * vec.getBlock(vec.blockNumber).timestamp;
      console.log("at block: " + vec.blockNumber + " (" + new Date(ts) + ")");
    })();
  `)
	if modules, err := self.supportedApis(); err == nil {
...
func (js *jsre) apiBindings(f xvec.Frontend) error {
	...
	apiImpl, err := api.ParseApiString(strings.Join(apiNames, ","), codec.JSON, js.xvec, js.vector)
	if err != nil {
		utils.Fatalf("Unable to determine supported api's: %v", err)
	}

	jvec := rpc.NewJvec(api.Merge(apiImpl...), js.re, js.client, f)
	js.re.Set("jvec", struct{}{})
	t, _ := js.re.Get("jvec")
	jvecObj := t.Object()

	jvecObj.Set("send", jvec.Send)
	jvecObj.Set("sendAsync", jvec.Send)
	...
	err = js.re.Compile("vector.js", re.Web3_JS)
	if err != nil {
	...
	_, err = js.re.Run("web3.setProvider(jvec)")
	if err != nil {
		utils.Fatalf("Error setting web3 provider: %v", err)
	}

	// load only supported API's in javascript runtime
	shortcuts := "var vec = web3.vec; "
	...
	js.re.Run(`var GlobalRegistrar = vec.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	return nil
}
...
func (self *jsre) ConfirmTransaction(tx string) bool {
	if self.vector.NatSpec {
		notice := natspec.GetNotice(self.xvec, tx, self.vector.HTTPClient())
		fmt.Println(notice)
...
func (self *jsre) UnlockAccount(addr []byte) bool {
	...
	// TODO: allow retry
	if err := self.vector.AccountManager().Unlock(common.BytesToAddress(addr), pass); err != nil {
		return false
	} else {
