...
// gvec is the official command-line client for Vector.
package main
...
import (
	...
	"github.com/vector/go-vector/params"
...
func init() {
	...
	app = utils.NewApp(Version, "the go-vector command line interface")
	app.Action = run
...
`,
		...
		monitorCommand,
		{
			Action: makedag,
			Name:   "makedag",
			Usage:  "generate vecash dag (for testing)",
			Description: `
The makedag command generates an vecash DAG in /tmp/dag.
...
`,
		},
		{
			Action: version,
			Name:   "version",
			Usage:  "print vector version numbers",
			Description: `
...
`,
		...
		{
			Name:  "wallet",
			Usage: "vector presale wallet",
			Subcommands: []cli.Command{
				{
					Action: importWallet,
					Name:   "import",
					Usage:  "import vector presale wallet",
				},
			},
			Description: `
...
will prompt for your password and imports your vec presale account.
...
between vector nodes by simply copying.
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    vector --password <passwordfile> account new
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    vector --password <passwordfile> account update <address>
...
For non-interactive use the passphrase can be specified with the -password flag:
    ...
    vector --password <passwordfile> account import <keyfile>

Note:
As you can directly copy your encrypted accounts to another vector instance,
...
nodes.
		...
		},
		{
			Action: console,
			Name:   "console",
			Usage:  `Gvec Console: interactive JavaScript environment`,
			Description: `
The Gvec console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/vectordev1/go-vector/wiki/Javascipt-Console
`},
		{
			Action: attach,
			Name:   "attach",
			Usage:  `Gvec Console: interactive JavaScript environment (connect to node)`,
			Description: `
The Gvec console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/vectordev1/go-vector/wiki/Javascipt-Console.
This command allows to open a console on a running gvec node.
`,
		},
		{
			Action: execJSFiles,
			Name:   "js",
			Usage:  `executes the given JavaScript files in the Gvec JavaScript VM`,
			Description: `
The JavaScript VM exposes a node admin interface as well as the Ðapp
JavaScript API. See https://github.com/vectordev1/go-vector/wiki/Javascipt-Console
`,
		},
	}
	app.Flags = []cli.Flag{
...
func attach(ctx *cli.Context) {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to gvec node - %v", err)
	}
...
func blockRecovery(ctx *cli.Context) {
	...
	cfg := utils.MakeEthConfig(ClientIdentifier, nodeNameVersion, ctx)
	blockDb, err := vecdb.NewLDBDatabase(filepath.Join(cfg.DataDir, "blockchain"), cfg.DatabaseCache)
	if err != nil {
...
func startEth(ctx *cli.Context, vec *vec.Vector) {
	// Start Vector itself
	utils.StartVector(vec)

	am := vec.AccountManager()
	...
	if !ctx.GlobalBool(utils.IPCDisabledFlag.Name) {
		if err := utils.StartIPC(vec, ctx); err != nil {
			utils.Fatalf("Error string IPC: %v", err)
		}
	}
	if ctx.GlobalBool(utils.RPCEnabledFlag.Name) {
		if err := utils.StartRPC(vec, ctx); err != nil {
...
func makedag(ctx *cli.Context) {
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: gvec makedag <block number> <outputdir>`)
	}
	switch {
	case len(args) == 2:
...
func gpuinfo(ctx *cli.Context) {
	vec.PrintOpenCLDevices()
}

func gpubench(ctx *cli.Context) {
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: gvec gpubench <gpu number>`)
	}
	switch {
	case len(args) == 1:
...
func version(c *cli.Context) {
	...
	fmt.Println("Protocol Versions:", vec.ProtocolVersions)
