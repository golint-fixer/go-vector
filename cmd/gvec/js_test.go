...
type testjvecre struct {
	*jsre
	lastConfirm string
	client      *httpclient.HTTPClient
}

func (self *testjvecre) UnlockAccount(acc []byte) bool {
	err := self.vector.AccountManager().Unlock(common.BytesToAddress(acc), "")
	if err != nil {
...
func (self *testjvecre) ConfirmTransaction(tx string) bool {
	if self.vector.NatSpec {
		self.lastConfirm = natspec.GetNotice(self.xvec, tx, self.client)
	}
	return true
}

func testJEthRE(t *testing.T) (string, *testjvecre, *vec.Vector) {
	return testREPL(t, nil)
}

func testREPL(t *testing.T, config func(*vec.Config)) (string, *testjvecre, *vec.Vector) {
	tmp, err := ioutil.TempDir("", "gvec-test")
	if err != nil {
		t.Fatal(err)
	}
	...
	conf := &vec.Config{
		...
		PowTest:        true,
		NewDB:          func(path string) (vecdb.Database, error) { return db, nil },
	}
	if config != nil {
		config(conf)
	}
	vector, err := vec.New(conf)
	if err != nil {
		t.Fatal("%v", err)
	}
	...
	assetPath := filepath.Join(os.Getenv("GOPATH"), "src", "github.com", "vector", "go-vector", "cmd", "mist", "assets", "ext")
	client := comms.NewInProcClient(codec.JSON)
	tf := &testjvecre{client: vector.HTTPClient()}
	repl := newJSRE(vector, assetPath, "", client, false, tf)
	tf.jsre = repl
	return tmp, tf, vector
}

func TestNodeInfo(t *testing.T) {
	t.Skip("broken after p2p update")
	tmp, repl, vector := testJEthRE(t)
	if err := vector.Start(); err != nil {
		t.Fatalf("error starting vector: %v", err)
	}
	defer vector.Stop()
...
func TestAccounts(t *testing.T) {
	...
	checkEvalJSON(t, repl, `vec.accounts`, `["`+testAddress+`"]`)
	checkEvalJSON(t, repl, `vec.coinbase`, `"`+testAddress+`"`)
	...
	checkEvalJSON(t, repl, `vec.accounts`, `["`+testAddress+`","`+addr+`"]`)
...
func TestBlockChain(t *testing.T) {
	...
	val, err := repl.re.Run("JSON.stringify(debug.dumpBlock(vec.blockNumber))")
	if err != nil {
		t.Errorf("expected no error, got %v", err)
	}
	...
	// do the export
	extmp, err := ioutil.TempDir("", "gvec-test-export")
	if err != nil {
		t.Fatal(err)
	}
	...
	checkEvalJSON(t, repl, `debug.dumpBlock(vec.blockNumber)`, beforeExport)
}
...
func TestMining(t *testing.T) {
	...
	defer vector.Stop()
	defer os.RemoveAll(tmp)
	checkEvalJSON(t, repl, `vec.mining`, `false`)
}
...
func TestRPC(t *testing.T) {
	...
	checkEvalJSON(t, repl, `admin.startRPC("127.0.0.1", 5004, "*", "web3,vec,net")`, `true`)
}
...
func TestCheckTestAccountBalance(t *testing.T) {
	...
	checkEvalJSON(t, repl, `vec.getBalance(primary)`, `"`+testBalance+`"`)
}
...
func TestSignature(t *testing.T) {
	...
	val, err := repl.re.Run(`vec.sign("` + testAddress + `", "` + testHash + `")`)
...
func TestContract(t *testing.T) {
	t.Skip("contract testing is implemented with mining in vecash test mode. This takes about 7seconds to run. Unskip and run on demand")
	coinbase := common.HexToAddress(testAddress)
	tmp, repl, vector := testREPL(t, func(conf *vec.Config) {
		conf.Vecbase = coinbase
		conf.PowTest = true
	})
	if err := vector.Start(); err != nil {
		t.Errorf("error starting vector: %v", err)
	...
	* then set once more SetHashReg SetUrlHint
	...
	if checkEvalJSON(t, repl, `primary = vec.accounts[0]`, `"`+testAddress+`"`) != nil {
		return
	}
	...
	if err != nil {
		...
		if checkEvalJSON(t, repl, `contract = vec.compile.solidity(source).test`, string(contractInfo)) != nil {
			return
		}
	}
	...
	if checkEvalJSON(
		t, repl,
		`contractaddress = vec.sendTransaction({from: primary, data: contract.code})`,
...
func pendingTransactions(repl *testjvecre, t *testing.T) (txc int64, err error) {
	txs := repl.vector.TxPool().GetTransactions()
	return int64(len(txs)), nil
}

func processTxs(repl *testjvecre, t *testing.T, expTxc int) bool {
	var txc int64
	var err error
	for i := 0; i < 50; i++ {
	...
	err = repl.vector.StartMining(runtime.NumCPU(), "")
	if err != nil {
		t.Errorf("unexpected error mining: %v", err)
		return false
	}
	defer repl.vector.StopMining()

	timer := time.NewTimer(100 * time.Second)
	height := new(big.Int).Add(repl.xvec.CurrentBlock().Number(), big.NewInt(1))
	repl.wait <- height
	select {
	case <-timer.C:
		// if times out make sure the xvec loop does not block
		go func() {
			select {
			case repl.wait <- nil:
			case <-repl.wait:
...
func checkEvalJSON(t *testing.T, re *testjvecre, expr, want string) error {
