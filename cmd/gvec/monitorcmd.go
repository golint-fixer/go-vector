...
var (
	...
	monitorCommand = cli.Command{
		Action: monitor,
		Name:   "monitor",
		Usage:  `Gvec Monitor: node metrics monitoring and visualization`,
		Description: `
The Gvec monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) {
	...
	// Attach to an Vector node over IPC or RPC
	...
	xvec := rpc.NewXvec(client)

	// Retrieve all the available metrics and resolve the user pattens
	metrics, err := retrieveMetrics(xvec)
	if err != nil {
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by gvec (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
	...
	refreshCharts(xvec, monitored, data, units, charts, ctx, footer)
	termui.Body.Align()
	...
	for {
		...
		case <-refresh:
			if refreshCharts(xvec, monitored, data, units, charts, ctx, footer) {
				termui.Body.Align()
			}
			termui.Render(termui.Body)
		}
...
// retrieveMetrics contacts the attached gvec node and retrieves the entire set
// of collected system metrics.
func retrieveMetrics(xvec *rpc.Xvec) (map[string]interface{}, error) {
	return xvec.Call("debug_metrics", []interface{}{true})
}
...
func refreshCharts(xvec *rpc.Xvec, metrics []string, data [][]float64, units []int, charts []*termui.LineChart, ctx *cli.Context, footer *termui.Par) (realign bool) {
	values, err := retrieveMetrics(xvec)
