...
import (
	...
	"github.com/vector/go-vector/rpc/useragent"
...
// MakeEthConfig creates vector options from set command line flags.
func MakeEthConfig(clientID, version string, ctx *cli.Context) *vec.Config {
	customName := ctx.GlobalString(IdentityFlag.Name)
	if len(customName) > 0 {
		clientID += "/" + customName
	}
	am := MakeAccountManager(ctx)
	vecbase, err := ParamToAddress(ctx.GlobalString(VecbaseFlag.Name), am)
	if err != nil {
		glog.V(logger.Error).Infoln("WARNING: No vecbase set and no accounts found as default")
	}
	// Assemble the entire vec configuration and return
	cfg := &vec.Config{
		...
		Vecbase:               common.HexToAddress(vecbase),
	...
	if ctx.GlobalBool(DevModeFlag.Name) {
		...
		if !ctx.GlobalIsSet(DataDirFlag.Name) {
			cfg.DataDir = os.TempDir() + "/vector_dev_mode"
		}
		cfg.PowTest = true
		cfg.DevMode = true
...
func MakeChain(ctx *cli.Context) (chain *core.BlockChain, chainDb vecdb.Database) {
	datadir := MustDataDir(ctx)
	cache := ctx.GlobalInt(CacheFlag.Name)

	var err error
	if chainDb, err = vecdb.NewLDBDatabase(filepath.Join(datadir, "chaindata"), cache); err != nil {
...
func IpcSocketPath(ctx *cli.Context) (ipcpath string) {
	if runtime.GOOS == "windows" {
		...
		ipcpath = common.DefaultIpcPath()
		if ctx.GlobalIsSet(DataDirFlag.Name) {
			ipcpath = filepath.Join(ctx.GlobalString(DataDirFlag.Name), "gvec.ipc")
		}
		if ctx.GlobalIsSet(IPCPathFlag.Name) {
			ipcpath = ctx.GlobalString(IPCPathFlag.Name)
		}
...
func StartIPC(vec *vec.Vector, ctx *cli.Context) error {
	config := comms.IpcConfig{
		Endpoint: IpcSocketPath(ctx),
	}

	initializer := func(conn net.Conn) (comms.Stopper, shared.VectorApi, error) {
		fe := useragent.NewRemoteFrontend(conn, vec.AccountManager())
		xvec := xvec.New(vec, fe)
		apis, err := api.ParseApiString(ctx.GlobalString(IPCApiFlag.Name), codec.JSON, xvec, vec)
		if err != nil {
			return nil, nil, err
		}
		return xvec, api.Merge(apis...), nil
	}

	return comms.StartIpc(config, codec.JSON, initializer)
}

func StartRPC(vec *vec.Vector, ctx *cli.Context) error {
	...
	xvec := xvec.New(vec, nil)
	codec := codec.JSON

	apis, err := api.ParseApiString(ctx.GlobalString(RpcApiFlag.Name), codec, xvec, vec)
	if err != nil {
		return err
	}
