...
// Package utils contains internal helper functions for go-vector commands.
package utils
...
func StartVector(vector *vec.Vector) {
	glog.V(logger.Info).Infoln("Starting", vector.Name())
	if err := vector.Start(); err != nil {
		Fatalf("Error starting Vector: %v", err)
	}
	go func() {
