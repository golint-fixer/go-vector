...
// PeerInfo represents a short summary of the Vector sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Vector protocol version negotiated
...
// Handshake executes the vec protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("vec/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Vector sub-protocol.
type peerSet struct {
