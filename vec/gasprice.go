...
func NewGasPriceOracle(vec *Vector) *GasPriceOracle {
	minprice := vec.GpoMinGasPrice
	if minprice == nil {
		minprice = big.NewInt(gpoDefaultMinGasPrice)
	}
	minbase := new(big.Int).Mul(minprice, big.NewInt(100))
	if vec.GpobaseCorrectionFactor > 0 {
		minbase = minbase.Div(minbase, big.NewInt(int64(vec.GpobaseCorrectionFactor)))
	}
...
func (gpo *GasPriceOracle) init() {
	gpo.initOnce.Do(func() {
		gpo.processPastBlocks(gpo.vec.BlockChain())
		go gpo.listenLoop()
	})
}
...
func (self *GasPriceOracle) listenLoop() {
	events := self.vec.EventMux().Subscribe(core.ChainEvent{}, core.ChainSplitEvent{})
...
func (self *GasPriceOracle) processBlock(block *types.Block) {
	...
	if lastBase.Cmp(lp) < 0 {
		corr = self.vec.GpobaseStepUp
	} else {
		corr = -self.vec.GpobaseStepDown
	}
...
func (self *GasPriceOracle) lowestPrice(block *types.Block) *big.Int {
	...
	receipts := core.GetBlockReceipts(self.vec.ChainDb(), block.Hash())
	if len(receipts) > 0 {
	...
	if new(big.Int).Mul(gasUsed, big.NewInt(100)).Cmp(new(big.Int).Mul(block.GasLimit(),
		big.NewInt(int64(self.vec.GpoFullBlockRatio)))) < 0 {
...
func (self *GasPriceOracle) SuggestPrice() *big.Int {
	...
	price.Mul(price, big.NewInt(int64(self.vec.GpobaseCorrectionFactor)))
	price.Div(price, big.NewInt(100))
	if price.Cmp(self.minPrice) < 0 {
		price.Set(self.minPrice)
	} else if self.vec.GpoMaxGasPrice != nil && price.Cmp(self.vec.GpoMaxGasPrice) > 0 {
		price.Set(self.vec.GpoMaxGasPrice)
	}
	return price
}
