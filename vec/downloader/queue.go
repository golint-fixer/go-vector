...
type fetchRequest struct {
	...
	Hashes  map[common.Hash]int // [vec/61] Requested hashes with their insertion index (priority)
	Headers []*types.Header     // [vec/62] Requested headers, sorted by request order
...
type queue struct {
	...
	hashPool    map[common.Hash]int // [vec/61] Pending hashes, mapping to their insertion index (priority)
	hashQueue   *prque.Prque        // [vec/61] Priority queue of the block hashes to fetch
	hashCounter int                 // [vec/61] Counter indexing the added hashes to ensure retrieval order

	headerHead common.Hash // [vec/62] Hash of the last queued header to verify order

	blockTaskPool  map[common.Hash]*types.Header // [vec/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [vec/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [vec/62] Currently pending block (body) retrieval operations
	blockDonePool  map[common.Hash]struct{}      // [vec/62] Set of the completed block (body) fetches

	receiptTaskPool  map[common.Hash]*types.Header // [vec/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [vec/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [vec/63] Currently pending receipt retrieval operations
	receiptDonePool  map[common.Hash]struct{}      // [vec/63] Set of the completed receipt fetches

	stateTaskIndex int                      // [vec/63] Counter indexing the added hashes to ensure prioritised retrieval order
	stateTaskPool  map[common.Hash]int      // [vec/63] Pending node data retrieval tasks, mapping to their priority
	stateTaskQueue *prque.Prque             // [vec/63] Priority queue of the hashes to fetch the node data for
	statePendPool  map[string]*fetchRequest // [vec/63] Currently pending node data retrieval operations

	stateDatabase   vecdb.Database   // [vec/63] Trie database to populate during state reassembly
	stateScheduler  *state.StateSync // [vec/63] State trie synchronisation scheduler and integrator
	stateProcessors int32            // [vec/63] Number of currently running state processors
	stateSchedLock  sync.RWMutex     // [vec/63] Lock serialising access to the state scheduler
...
func newQueue(stateDb vecdb.Database) *queue {
	lock := new(sync.Mutex)
	return &queue{
...
// InFlightBlocks retrieves whvec there are block fetch requests currently in
// flight.
...
// InFlightReceipts retrieves whvec there are receipt fetch requests currently
// in flight.
...
// InFlightNodeData retrieves whvec there are node data entry fetch requests
// currently in flight.
...
func (q *queue) reserveHashes(p *peer, count int, taskQueue *prque.Prque, taskGen func(int), pendPool map[string]*fetchRequest, maxPending int) *fetchRequest {
	// Short circuit if the peer's already downloading somvecing (sanity check to
	// not corrupt state)
...
// returns a flag whvec empty blocks were queued requiring processing.
...
// also returns a flag whvec empty receipts were queued requiring importing.
...
func (q *queue) reserveHeaders(p *peer, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	...
	// downloading somvecing (sanity check not to corrupt state)
	if taskQueue.Empty() {
