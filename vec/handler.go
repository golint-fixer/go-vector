...
import (
	...
	"github.com/vector/go-vector/vec/fetcher"
	...
	"github.com/vector/go-vector/pow"
...
// NewProtocolManager returns a new vector sub protocol manager. The Vector sub protocol manages peers capable
// with the vector network.
func NewProtocolManager(fastSync bool, networkId int, mux *event.TypeMux, txpool txPool, pow pow.PoW, blockchain *core.BlockChain, chaindb vecdb.Database) (*ProtocolManager, error) {
	// Figure out whvec to allow fast sync or not
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	// Unregister the peer from the downloader and Vector peer set
...
func (pm *ProtocolManager) Stop() {
	...
	glog.V(logger.Info).Infoln("Stopping vector protocol handler...")
	...
	glog.V(logger.Info).Infoln("Vector protocol handler stopped")
}
...
// handle is the callback invoked to manage the life cycle of an vec peer. When
// this function terminates, the peer is disconnected.
func (pm *ProtocolManager) handle(p *peer) error {
	glog.V(logger.Debug).Infof("%v: peer connected [%s]", p, p.Name())

	// Execute the Vector handshake
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version < vec62 && msg.Code == GetBlockHashesMsg:
	...
	case p.version < vec62 && msg.Code == GetBlockHashesFromNumberMsg:
	...
	case p.version < vec62 && msg.Code == BlockHashesMsg:
	...
	case p.version < vec62 && msg.Code == GetBlocksMsg:
	...
	case p.version < vec62 && msg.Code == BlocksMsg:
	...
	case p.version >= vec62 && msg.Code == GetBlockHeadersMsg:
	...
	case p.version >= vec62 && msg.Code == BlockHeadersMsg:
	...
	case p.version >= vec62 && msg.Code == GetBlockBodiesMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= vec62 && msg.Code == BlockBodiesMsg:
	...
	case p.version >= vec63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= vec63 && msg.Code == NodeDataMsg:
	...
	case p.version >= vec63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= vec63 && msg.Code == ReceiptsMsg:
...
// EthNodeInfo represents a short summary of the Vector sub-protocol metadata known
// about the host peer.
type EthNodeInfo struct {
	Network    int      `json:"network"`    // Vector network ID (0=Olympic, 1=Frontier, 2=Morden)
