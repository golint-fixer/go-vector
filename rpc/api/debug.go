...
func NewDebugApi(xvec *xvec.XEth, vector *vec.Vector, coder codec.Codec) *debugApi {
	return &debugApi{
		xvec:     xvec,
		vector: vector,
...
func (self *debugApi) DumpBlock(req *shared.Request) (interface{}, error) {
	...
	stateDb, err := state.New(block.Root(), self.vector.ChainDb())
	if err != nil {
		return nil, err
	}
...
func (self *debugApi) SetHead(req *shared.Request) (interface{}, error) {
	...
	self.vector.BlockChain().SetHead(uint64(args.BlockNumber))
...
func (self *debugApi) ProcessBlock(req *shared.Request) (interface{}, error) {
	...
	var (
		blockchain = self.vector.BlockChain()
	...
	statedb, err := state.New(blockchain.GetBlock(block.ParentHash()).Root(), self.vector.ChainDb())
	if err != nil {
		return false, err
	}
...
func (self *debugApi) SeedHash(req *shared.Request) (interface{}, error) {
	...
	if hash, err := vecash.GetSeedHash(uint64(args.BlockNumber)); err == nil {
