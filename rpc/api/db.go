...
func NewDbApi(xvec *xvec.XEth, vector *vec.Vector, coder codec.Codec) *dbApi {
	return &dbApi{
		xvec:     xvec,
		vector: vector,
		methods:  DbMapping,
...
func (self *dbApi) GetString(req *shared.Request) (interface{}, error) {
	...
	ret, err := self.xvec.DbGet([]byte(args.Database + args.Key))
...
func (self *dbApi) PutString(req *shared.Request) (interface{}, error) {
	...
	return self.xvec.DbPut([]byte(args.Database+args.Key), args.Value), nil
}
...
func (self *dbApi) GetHex(req *shared.Request) (interface{}, error) {
	...
	if res, err := self.xvec.DbGet([]byte(args.Database + args.Key)); err == nil {
