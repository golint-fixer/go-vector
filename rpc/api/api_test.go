...
func TestParseApiString(t *testing.T) {
	...
	apis, err = ParseApiString("vec", codec.JSON, nil, nil)
	if err != nil {
	...
	apis, err = ParseApiString("vec,vec", codec.JSON, nil, nil)
	if err != nil {
		t.Errorf("Expected nil err from parsing empty API string but got \"%v\"", err)
	}
	...
	apis, err = ParseApiString("vec,invalid", codec.JSON, nil, nil)
	if err == nil {
...
func TestCompileSolidity(t *testing.T) {
	...
	vec := &vec.Vector{}
	xvec := xvec.NewTest(vec, nil)
	api := NewEthApi(xvec, vec, codec.JSON)
