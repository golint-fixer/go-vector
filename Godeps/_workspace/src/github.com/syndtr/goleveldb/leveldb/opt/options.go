...
type Options struct {
	...
	// ErrorIfExist defines whvec an error should returned if the DB already
	// exist.
	//
	// The default value is false.
	ErrorIfExist bool

	// ErrorIfMissing defines whvec an error should returned if the DB is
...
type ReadOptions struct {
	// DontFillCache defines whvec block reads for this 'read operation'
...
type WriteOptions struct {
	// Sync is whvec to sync underlying writes from the OS buffer cache
