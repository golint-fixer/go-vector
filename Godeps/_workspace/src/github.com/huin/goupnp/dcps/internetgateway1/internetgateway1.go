...
const (
	...
	URN_WANVecnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANVecnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Vecnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Vecnet
//
...
// WANVecnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANVecnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANVecnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANVecnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANVecnetLinkConfig1Clients() (clients []*WANVecnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANVecnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANVecnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANVecnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANVecnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANVecnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANVecnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANVecnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANVecnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANVecnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANVecnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANVecnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANVecnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANVecnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANVecnetLinkConfig1 {
	clients := make([]*WANVecnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANVecnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewVecnetLinkStatus: allowed values: Up, Down
func (client *WANVecnetLinkConfig1) GetVecnetLinkStatus() (NewVecnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewVecnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANVecnetLinkConfig_1, "GetVecnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewVecnetLinkStatus, err = soap.UnmarshalString(response.NewVecnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
