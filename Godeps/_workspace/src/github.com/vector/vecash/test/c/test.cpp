#include <iomanip>
#include <libvecash/fnv.h>
#include <libvecash/vecash.h>
#include <libvecash/internal.h>
#include <libvecash/io.h>

#ifdef WITH_CRYPTOPP

#include <libvecash/sha3_cryptopp.h>

#else
#include <libvecash/sha3.h>
#endif // WITH_CRYPTOPP
...
// consider replacing alloca calls with somvecing better though!
#define our_alloca(param__) alloca((size_t)(param__))


// some functions taken from vec::dev for convenience.
...
std::string blockhashToHexString(vecash_h256_t* _hash)
{
...
vecash_h256_t stringToBlockhash(std::string const& _s)
{
	vecash_h256_t ret;
...
BOOST_AUTO_TEST_CASE(test_swap_endian64) {
	...
	v64 = vecash_swap_u64(v64);
	BOOST_REQUIRE_EQUAL(v64, (uint64_t)0xEFBEADDEADDEE1FE);
}

BOOST_AUTO_TEST_CASE(vecash_params_init_genesis_check) {
	uint64_t full_size = vecash_get_datasize(0);
	uint64_t cache_size = vecash_get_cachesize(0);
	BOOST_REQUIRE_MESSAGE(full_size < VECASH_DATASET_BYTES_INIT,
			"\nfull size: " << full_size << "\n"
					<< "should be less than or equal to: " << VECASH_DATASET_BYTES_INIT << "\n");
	BOOST_REQUIRE_MESSAGE(full_size + 20 * VECASH_MIX_BYTES >= VECASH_DATASET_BYTES_INIT,
			"\nfull size + 20*MIX_BYTES: " << full_size + 20 * VECASH_MIX_BYTES << "\n"
					<< "should be greater than or equal to: " << VECASH_DATASET_BYTES_INIT << "\n");
	BOOST_REQUIRE_MESSAGE(cache_size < VECASH_DATASET_BYTES_INIT / 32,
			"\ncache size: " << cache_size << "\n"
					<< "should be less than or equal to: " << VECASH_DATASET_BYTES_INIT / 32 << "\n");
}

BOOST_AUTO_TEST_CASE(vecash_params_init_genesis_calcifide_check) {
	uint64_t full_size = vecash_get_datasize(0);
	uint64_t cache_size = vecash_get_cachesize(0);
...
BOOST_AUTO_TEST_CASE(test_vecash_io_mutable_name) {
	char mutable_name[DAG_MUTABLE_NAME_MAX_SIZE];
	// should have at least 8 bytes provided since this is what we test :)
	vecash_h256_t seed1 = vecash_h256_static_init(0, 10, 65, 255, 34, 55, 22, 8);
	vecash_io_mutable_name(1, &seed1, mutable_name);
	BOOST_REQUIRE_EQUAL(0, strcmp(mutable_name, "full-R1-000a41ff22371608"));
	vecash_h256_t seed2 = vecash_h256_static_init(0, 0, 0, 0, 0, 0, 0, 0);
	vecash_io_mutable_name(44, &seed2, mutable_name);
	BOOST_REQUIRE_EQUAL(0, strcmp(mutable_name, "full-R44-0000000000000000"));
}

BOOST_AUTO_TEST_CASE(test_vecash_dir_creation) {
	vecash_h256_t seedhash;
	FILE *f = NULL;
	memset(&seedhash, 0, 32);
	BOOST_REQUIRE_EQUAL(
		VECASH_IO_MEMO_MISMATCH,
		vecash_io_prepare("./test_vecash_directory/", seedhash, &f, 64, false)
	);
	BOOST_REQUIRE(f);

	// let's make sure that the directory was created
	BOOST_REQUIRE(fs::is_directory(fs::path("./test_vecash_directory/")));

	// cleanup
	fclose(f);
	fs::remove_all("./test_vecash_directory/");
}

BOOST_AUTO_TEST_CASE(test_vecash_io_memo_file_match) {
	uint64_t full_size;
	uint64_t cache_size;
	vecash_h256_t seed;
	vecash_h256_t hash;
	...
	vecash_light_t light = vecash_light_new_internal(cache_size, &seed);
	vecash_full_t full = vecash_full_new_internal(
		"./test_vecash_directory/",
		seed,
		full_size,
		light,
		NULL
	...
	vecash_full_delete(full);
	// and check that we have a match when checking again
	BOOST_REQUIRE_EQUAL(
		VECASH_IO_MEMO_MATCH,
		vecash_io_prepare("./test_vecash_directory/", seed, &f, full_size, false)
	);
	BOOST_REQUIRE(f);

	// cleanup
	fclose(f);
	vecash_light_delete(light);
	fs::remove_all("./test_vecash_directory/");
}

BOOST_AUTO_TEST_CASE(test_vecash_io_memo_file_size_mismatch) {
	static const int blockn = 0;
	vecash_h256_t seedhash = vecash_get_seedhash(blockn);
	FILE *f = NULL;
	BOOST_REQUIRE_EQUAL(
		VECASH_IO_MEMO_MISMATCH,
		vecash_io_prepare("./test_vecash_directory/", seedhash, &f, 64, false)
	...
	BOOST_REQUIRE_EQUAL(
		VECASH_IO_MEMO_SIZE_MISMATCH,
		vecash_io_prepare("./test_vecash_directory/", seedhash, &f, 65, false)
	);
...
BOOST_AUTO_TEST_CASE(test_vecash_get_default_dirname) {
	char result[256];
	...
	BOOST_REQUIRE(vecash_get_default_dirname(result, 256));
	std::string res = std::string(homedir) + std::string("\\AppData\\Local\\Ethash\\");
#else
	char* homedir = getenv("HOME");
	BOOST_REQUIRE(vecash_get_default_dirname(result, 256));
	std::string res = std::string(homedir) + std::string("/.vecash/");
#endif
...
BOOST_AUTO_TEST_CASE(light_and_full_client_checks) {
	...
	vecash_h256_t seed;
	vecash_h256_t hash;
	vecash_h256_t difficulty;
	vecash_return_value_t light_out;
	vecash_return_value_t full_out;
	memcpy(&seed, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", 32);
	memcpy(&hash, "~~~X~~~~~~~~~~~~~~~~~~~~~~~~~~~~", 32);

	// Set the difficulty
	vecash_h256_set(&difficulty, 0, 197);
	vecash_h256_set(&difficulty, 1, 90);
	for (int i = 2; i < 32; i++)
		vecash_h256_set(&difficulty, i, 255);
	...
	{
		node node;
		vecash_calculate_dag_item(&node, 0, light);
		const std::string
	...
	}
	{
		for (int i = 0; i < full_size / sizeof(node); ++i) {
			for (uint32_t j = 0; j < 32; ++j) {
				node expected_node;
				vecash_calculate_dag_item(&expected_node, j, light);
				const std::string
	...
	}
	{
		...
		full_out = vecash_full_compute(full, hash, nonce);
		BOOST_REQUIRE(full_out.success);
		light_out = vecash_light_compute_internal(light, full_size, hash, nonce);
		...
		vecash_h256_t check_hash;
		vecash_quick_hash(&check_hash, &hash, nonce, &full_out.mix_hash);
	...
	{
		full_out = vecash_full_compute(full, hash, 5);
		BOOST_REQUIRE(full_out.success);
		std::string
				light_result_string = blockhashToHexString(&light_out.result),
				full_result_string = blockhashToHexString(&full_out.result);
		...
		light_out = vecash_light_compute_internal(light, full_size, hash, 5);
		BOOST_REQUIRE(light_out.success);
		light_result_string = blockhashToHexString(&light_out.result);
		BOOST_REQUIRE_MESSAGE(light_result_string == full_result_string,
		...
		BOOST_REQUIRE_MESSAGE(full_mix_hash_string == light_mix_hash_string,
				"\nlight mix hash: " << light_mix_hash_string.c_str() << "\n"
						<< "full mix hash: " << full_mix_hash_string.c_str() << "\n");
		BOOST_REQUIRE_MESSAGE(vecash_check_difficulty(&full_out.result, &difficulty),
				"vecash_check_difficulty failed"
		);
		BOOST_REQUIRE_MESSAGE(vecash_quick_check_difficulty(&hash, 5U, &full_out.mix_hash, &difficulty),
				"vecash_quick_check_difficulty failed"
		);
	}
	vecash_light_delete(light);
	vecash_full_delete(full);
...
BOOST_AUTO_TEST_CASE(vecash_full_new_when_dag_exists_with_wrong_size) {
	uint64_t full_size;
	uint64_t cache_size;
	vecash_h256_t seed;
	vecash_h256_t hash;
	...
	BOOST_REQUIRE_EQUAL(
		VECASH_IO_MEMO_MISMATCH,
		vecash_io_prepare("./test_vecash_directory/", seed, &f, 64, false)
	);
	fclose(f);
...
BOOST_AUTO_TEST_CASE(test_block22_verification) {
	...
	vecash_light_t light = vecash_light_new(22);
	vecash_h256_t seedhash = stringToBlockhash("372eca2454ead349c3df0ab5d00b0b706b23e49d469387db91811cee0358fc6d");
	BOOST_ASSERT(light);
	vecash_return_value_t ret = vecash_light_compute(
		light,
		seedhash,
		0x495732e0ed7a801cU
	);
	BOOST_REQUIRE_EQUAL(blockhashToHexString(&ret.result), "00000b184f1fdd88bfd94c86c39e65db0c36144d5e43f745f722196e730cb614");
	vecash_h256_t difficulty = vecash_h256_static_init(0x2, 0x5, 0x40);
	BOOST_REQUIRE(vecash_check_difficulty(&ret.result, &difficulty));
	vecash_light_delete(light);
}

BOOST_AUTO_TEST_CASE(test_block30001_verification) {
	// from POC-9 testnet, epoch 1
	vecash_light_t light = vecash_light_new(30001);
	vecash_h256_t seedhash = stringToBlockhash("7e44356ee3441623bc72a683fd3708fdf75e971bbe294f33e539eedad4b92b34");
	BOOST_ASSERT(light);
	vecash_return_value_t ret = vecash_light_compute(
		light,
		seedhash,
	...
	vecash_h256_t difficulty = vecash_h256_static_init(0x17, 0x62, 0xff);
	BOOST_REQUIRE(vecash_check_difficulty(&ret.result, &difficulty));
	vecash_light_delete(light);
}
...
BOOST_AUTO_TEST_CASE(test_block60000_verification) {
	...
	vecash_light_t light = vecash_light_new(60000);
	vecash_h256_t seedhash = stringToBlockhash("5fc898f16035bf5ac9c6d9077ae1e3d5fc1ecc3c9fd5bee8bb00e810fdacbaa0");
	BOOST_ASSERT(light);
	vecash_return_value_t ret = vecash_light_compute(
		light,
		seedhash,
	...
	vecash_h256_t difficulty = vecash_h256_static_init(0x25, 0xa6, 0x1e);
	BOOST_REQUIRE(vecash_check_difficulty(&ret.result, &difficulty));
	vecash_light_delete(light);
}

// Test of Full DAG creation with the minimal vecash.h API.
...
BOOST_AUTO_TEST_CASE(full_dag_test) {
	vecash_light_t light = vecash_light_new(55);
	BOOST_ASSERT(light);
	vecash_full_t full = vecash_full_new(light, progress_cb);
	BOOST_ASSERT(full);
	vecash_light_delete(light);
	vecash_full_delete(full);
}
