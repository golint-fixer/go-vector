...
/*
#include "src/libvecash/internal.h"

int vecashGoCallback_cgo(unsigned);
*/
import "C"
...
func defaultDir() string {
	...
	return filepath.Join(home, ".vecash")
}

// cache wraps an vecash_light_t with some metadata
...
func (cache *cache) generate() {
	cache.gen.Do(func() {
		...
		size := C.vecash_get_cachesize(C.uint64_t(cache.epoch * epochLength))
		if cache.test {
			size = cacheSizeForTesting
		}
		cache.ptr = C.vecash_light_new_internal(size, (*C.vecash_h256_t)(unsafe.Pointer(&seedHash[0])))
...
func freeCache(cache *cache) {
	C.vecash_light_delete(cache.ptr)
	cache.ptr = nil
}
...
// Verify checks whvec the block's nonce is valid.
func (l *Light) Verify(block pow.Block) bool {
	// TODO: do vecash_quick_verify before getCache in order
	...
	/* Cannot happen if block header diff is validated prior to PoW, but can
	   ...
	   Vector protocol consensus rules here which are not in scope of Ethash
	*/
	...
	dagSize := C.vecash_get_datasize(C.uint64_t(blockNum))
	...
	ret := C.vecash_light_compute_internal(cache.ptr, dagSize, hash, C.uint64_t(block.Nonce()))
	if !ret.success {
		return false
	}
...
func h256ToHash(in C.vecash_h256_t) common.Hash {
	return *(*common.Hash)(unsafe.Pointer(&in.b))
}

func hashToH256(in common.Hash) C.vecash_h256_t {
	return C.vecash_h256_t{b: *(*[32]C.uint8_t)(unsafe.Pointer(&in[0]))}
}
...
// dag wraps an vecash_full_t with some metadata
// and automatic memory management.
type dag struct {
	epoch uint64
	test  bool
...
func (d *dag) generate() {
	d.gen.Do(func() {
		var (
			...
			cacheSize = C.vecash_get_cachesize(blockNum)
			dagSize   = C.vecash_get_datasize(blockNum)
		)
		if d.test {
		...
		cache := C.vecash_light_new_internal(cacheSize, (*C.vecash_h256_t)(unsafe.Pointer(&seedHash[0])))
		defer C.vecash_light_delete(cache)
		// Generate the actual DAG.
		d.ptr = C.vecash_full_new_internal(
			C.CString(d.dir),
			hashToH256(seedHash),
			dagSize,
			cache,
			(C.vecash_callback_t)(unsafe.Pointer(C.vecashGoCallback_cgo)),
		)
		if d.ptr == nil {
			panic("vecash_full_new IO or memory error")
		}
...
//export vecashGoCallback
func vecashGoCallback(percent C.unsigned) C.int {
...
func (pow *Full) Search(block pow.Block, stop <-chan struct{}, index int) (nonce uint64, mixDigest []byte) {
	...
	for {
		...
		default:
			...
			ret := C.vecash_full_compute(dag.ptr, hash, C.uint64_t(nonce))
...
func NewForTesting() (*Ethash, error) {
	dir, err := ioutil.TempDir("", "vecash-test")
	if err != nil {
		return nil, err
	}
