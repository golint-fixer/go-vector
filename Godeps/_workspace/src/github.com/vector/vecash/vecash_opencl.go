...
/*
  ...
  1. https://github.com/vector/wiki/wiki/Ethash
  2. https://github.com/vector/cpp-vector/blob/develop/libvecash-cl/vecash_cl_miner.cpp
...
type OpenCLMiner struct {
	...
	vecash *Ethash // Ethash full DAG & cache in host mem
...
func PrintDevices() {
	...
	if len(found) == 0 {
		...
		fmt.Printf("Found %v devices. Benchmark first GPU:       gvec gpubench 0\n", len(found))
		fmt.Printf("Mine using all GPUs:                        gvec --minegpu %v\n", idsFormat)
	}
}
...
func InitCL(blockNum uint64, c *OpenCLMiner) error {
	platforms, err := cl.GetPlatforms()
	if err != nil {
		return fmt.Errorf("Plaform error: %v\nCheck your OpenCL installation and then run gvec gpuinfo", err)
	}
	...
	for _, p := range platforms {
		...
		if err != nil {
			return fmt.Errorf("Devices error: %v\nCheck your GPU drivers and then run gvec gpuinfo", err)
		}
		for _, d := range ds {
	...
	c.vecash = pow
	dagSize := uint64(C.vecash_get_datasize(C.uint64_t(blockNum)))
	c.dagSize = dagSize

	for _, id := range c.deviceIds {
		if id > len(devices)-1 {
			return fmt.Errorf("Device id not found. See available device ids with: gvec gpuinfo")
		} else {
...
func initCLDevice(deviceId int, device *cl.Device, c *OpenCLMiner) error {
	...
	kvs["DAG_SIZE"] = strconv.FormatUint(c.dagSize/vecashMixBytesLen, 10)
	kvs["ACCESSES"] = strconv.FormatUint(vecashAccesses, 10)
	...
	*/
	...
	searchKernelName = "vecash_search"
	hashKernelName = "vecash_hash"
	...
	dagPtr := unsafe.Pointer(c.vecash.Full.current.ptr.data)
...
func (c *OpenCLMiner) Search(block pow.Block, stop <-chan struct{}, index int) (uint64, []byte) {
	c.mu.Lock()
	newDagSize := uint64(C.vecash_get_datasize(C.uint64_t(block.NumberU64())))
		...
		if len(pending) == searchBufSize {
			...
			for i := uint32(0); i < nfound; i++ {
				...
				if checkNonce != 0 {
					...
					ret := C.vecash_light_compute_internal(c.vecash.Light.current.ptr, ds, hashToH256(headerHash), cn)
...
func (c *OpenCLMiner) Verify(block pow.Block) bool {
	return c.vecash.Light.Verify(block)
}
