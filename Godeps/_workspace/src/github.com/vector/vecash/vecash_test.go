...
func TestEthashConcurrentSearch(t *testing.T) {
	...
	for i := 0; i < nsearch; i++ {
		go func() {
			nonce, md := vec.Search(block, stop, 0)
			select {
...
func TestGetSeedHash(t *testing.T) {
	...
	// From python:
	// > from pyvecash import get_seedhash
	// > get_seedhash(30000)
