/*
  This file is part of cpp-vector.

  cpp-vector is free software: you can redistribute it and/or modify
  it under the terms of the GNU General Public License as published by
  the Free Software Foundation, either version 3 of the License, or
  (at your option) any later version.

  cpp-vector is distributed in the hope that it will be useful,
...
#include <libvecash/util.h>
#ifdef OPENCL
#include <libvecash-cl/vecash_cl_miner.h>
#endif
#include <vector>
#include <algorithm>
...
static std::string bytesToHexString(vecash_h256_t const *hash, unsigned size)
{
...
extern "C" int main(void)
{
...
#ifdef FULL
	{
		auto startTime = high_resolution_clock::now();
		vecash_return_value hash;
		vecash_full(&hash, full_mem, &params, &previous_hash, 0);
		auto time = std::chrono::duration_cast<std::chrono::milliseconds>(high_resolution_clock::now() - startTime).count();
		debugf("vecash_full test: %uns\n", (unsigned)time);
	}
#endif
	...
	{
		vecash_return_value hash;
		vecash_light(&hash, &cache, &params, &previous_hash, i);
	...
	#ifdef OPENCL
	{
		struct search_hook : vecash_cl_miner::search_hook
		{
			unsigned hash_count;
		...
		for (unsigned i = 0; i != hook.nonce_vec.size(); ++i)
		{
			...
			vecash_return_value hash;
			vecash_light(&hash, &cache, &params, &previous_hash, nonce);
	...
	#else
	{
		...
		for (int nonce = 0; nonce < trials; ++nonce)
		{
			vecash_return_value hash;
			#ifdef FULL
				vecash_full(&hash, full_mem, &params, &previous_hash, nonce);
			#else
				vecash_light(&hash, &cache, &params, &previous_hash, nonce);
			#endif // FULL
		}
	...
	unsigned read_size = VECASH_ACCESSES * VECASH_MIX_BYTES;
