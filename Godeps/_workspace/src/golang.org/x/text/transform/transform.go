...
type Transformer interface {
	...
	// atEOF argument tells whvec src represents the last bytes of the
	// input.
	//
	// Callers should always process the nDst bytes produced and account
	// for the nSrc bytes consumed before considering the error err.
	//
	// A nil error means that all of the transformed bytes (whvec freshly
	// transformed from src or left over from previous Transform calls)
	// were written to dst. A nil error can be returned regardless of
	// whvec atEOF is true. If err is nil then nSrc must equal len(src);
...
type Reader struct {
	...
	// transformComplete is whvec the transformation is complete,
	// regardless of whvec or not it was successful.
...
func (c *chain) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	...
	// The error returned by Transform determines whvec to increase or
