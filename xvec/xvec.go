...
// Package xvec is the interface to all Vector functionality.
package xvec
...
import (
	...
	"github.com/vector/go-vector/vec/filters"
...
type XEth struct {
	...
	gpo           *vec.GasPriceOracle
	state         *State
	whisper       *Whisper
	filterManager *filters.FilterSystem
}

func NewTest(vec *vec.Vector, frontend Frontend) *XEth {
	return &XEth{backend: vec, frontend: frontend}
}

// New creates an XEth that uses the given frontend.
// If a nil Frontend is provided, a default frontend which
// confirms all transactions will be used.
func New(vector *vec.Vector, frontend Frontend) *XEth {
	xvec := &XEth{
		backend:          vector,
		frontend:         frontend,
		quit:             make(chan struct{}),
		filterManager:    filters.NewFilterSystem(vector.EventMux()),
		logQueue:         make(map[int]*logQueue),
		blockQueue:       make(map[int]*hashQueue),
		transactionQueue: make(map[int]*hashQueue),
		messages:         make(map[int]*whisperFilter),
		agent:            miner.NewRemoteAgent(),
		gpo:              vec.NewGasPriceOracle(vector),
	}
	if vector.Whisper() != nil {
		xvec.whisper = NewWhisper(vector.Whisper())
	}
	vector.Miner().Register(xvec.agent)
	if frontend == nil {
		xvec.frontend = dummyFrontend{}
	}
	state, _ := xvec.backend.BlockChain().State()
	xvec.state = NewState(xvec, state)
	go xvec.start()
	return xvec
}
...
func (self *XEth) WithState(statedb *state.StateDB) *XEth {
	...
	xvec.state = NewState(xvec, statedb)
	return xvec
}
...
func (p *XEth) NewWhisperFilter(to, from string, topics [][]string) int {
	...
	// Callback to delegate core whisper messages to this xvec filter
	...
	// Initialize the core whisper filter and wrap into xvec
